...
func (s *Store) ListGrantees(ctx context.Context) ([]Grantee, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT g.id, g.name, g.created_at,
		       COALESCE(GROUP_CONCAT(e.email, X'1F'), '')
		FROM grantees g
		LEFT JOIN grantee_emails e ON e.grantee_id = g.id
	...
	for rows.Next() {
		...
		if err := rows.Scan(&g.ID, &g.Name, &created, &emailsBlob); err != nil {
			return nil, err
		}
		g.CreatedAt = time.Unix(created, 0)
		if emailsBlob != "" {
			g.Emails = strings.Split(emailsBlob, "\x1F")
		} else {
			g.Emails = []string{}
		}
		out = append(out, g)
...
// Pass cursor = the last id from the previous page to continue; pass 0 to start.
// Returns (rows, nextCursor). nextCursor == 0 means no more pages.
func (s *Store) ListMessagesByGrantee(ctx context.Context, granteeID string, since, until *time.Time, limit int, cursorID int64) ([]MessageSummary, int64, error) {
	...
	out, next, err := scanMessageSummaries(rows, limit)
	if err != nil {
		return nil, 0, err
	}
	return out, next, nil
...
// GetMessage looks up a message by either its numeric id (as a string) or by RFC 5322 Message-ID.
// Returns the message, its attachments, and its recipients.
func (s *Store) GetMessage(ctx context.Context, idOrMessageID string) (*Message, []Attachment, []Recipient, error) {
	...
	if n, err := strconv.ParseInt(idOrMessageID, 10, 64); err == nil {
		row = s.db.QueryRowContext(ctx, messageSelectByID, n)
	} else {
		row = s.db.QueryRowContext(ctx, messageSelectByMessageID, idOrMessageID)
	}
...
// GetThreadMessages returns the thread record and its messages in chronological order.
// Returns (nil, nil, nil) when the thread does not exist.
...
func (s *Store) SearchMessages(ctx context.Context, p SearchParams) ([]MessageSummary, int64, error) {
	...
	if p.From != "" {
		where = append(where, "m.from_addr LIKE ? COLLATE NOCASE")
		args = append(args, "%"+p.From+"%")
	}
	if p.Subject != "" {
		where = append(where, "m.subject LIKE ? COLLATE NOCASE")
		args = append(args, "%"+p.Subject+"%")
	}
	if p.Body != "" {
		where = append(where, "m.body_text LIKE ? COLLATE NOCASE")
